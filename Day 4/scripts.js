...
const inventors = [
  ...
  const bornInfifteen1 = inventors.filter(inventor =>{
     if(inventor.year >= 1500 && inventor.year <= 1599){
         return true;
     }
  });
  const  bornInfifteen2 = inventors.filter(inventor => inventor.year >= 1500 && inventor.year <= 1599)
  console.table(bornInfifteen1);
  console.table(bornInfifteen2);
...
return lastGuy > nextGuy ? 1 : -1;
  ...
  //Codigo so funciona nessa pagina web a  cima
 // const Category = document.querySelector('.mw-category');
  //const links = Array.from(Category.querySelectorAll('a'));
  //const de  = links.map(link => link.textContent).filter(streetNames => streetNames.includes('de'));
...
//Filtrar por letra específica
    var filteredByA = inventors.filter( inventor =>{
        if(inventor.first.includes('a')){
            return true;
        }
    });
